...
class ProductService {
     ...
     *
     * Backward compatible: if `page` is not provided, behaves exactly like before
     * (returns the full array of products) so existing callers (e.g. admin pages
     * that need the whole list for client-side search/filter) keep working.
     *
     * When `page` IS provided, returns `{ data, pagination }` so list pages can
     * avoid loading the entire product collection at once.
     */
    async getAllProduct({ page, limit = 12, category, search } = {}) {
        const filter = {};
        if (category) filter.category = category;
        if (search) filter.name = { $regex: search, $options: 'i' };

        if (!page) {
            // Legacy behaviour: return the full (optionally filtered) list.
            return modelProduct.find(filter).populate('category');
        }
